...
class TestInitScript:
    ...
    def test_init_creates_repository(self, project_root, test_repo_path, cleanup_test_repo):
    ...
    def test_init_creates_directory_structure(self, project_root, test_repo_path, cleanup_test_repo):
        """Test that init script creates the expected directory structure."""
        script_path = project_root / "scripts" / "init_test_artifacts_repo.sh"

        subprocess.run([str(script_path)], cwd=str(project_root), check=True, capture_output=True)

        # Check directories
        assert (test_repo_path / "artifacts").exists(), "artifacts directory not created"
    ...
    def test_init_creates_gitignore(self, project_root, test_repo_path, cleanup_test_repo):
    ...
    def test_init_creates_initial_commit(self, project_root, test_repo_path, cleanup_test_repo):
    ...
    def test_init_creates_test_main_branch(self, project_root, test_repo_path, cleanup_test_repo):
        ...
        assert "test-main" in result.stdout, "test-main branch not created or not checked out"
...
class TestCleanupScript:
    ...
    def test_cleanup_removes_repository(self, project_root, test_repo_path, cleanup_test_repo):
        """Test that cleanup script removes the repository."""
        # First create the repository
        init_script = project_root / "scripts" / "init_test_artifacts_repo.sh"
        subprocess.run([str(init_script)], cwd=str(project_root), check=True, capture_output=True)
    ...
    def test_cleanup_handles_nonexistent_repo(self, project_root, test_repo_path, cleanup_test_repo):
        ...
        assert result.returncode == 0, "Cleanup should succeed even if repo doesn't exist"
        assert "No test artifacts repository found" in result.stdout or \
               "Nothing to clean up" in result.stdout, \
...
class TestRepositoryIsolation:
    ...
    def test_test_repo_not_tracked_by_main_repo(self, project_root, test_repo_path, cleanup_test_repo):
    ...
    def test_test_repo_has_separate_git(self, project_root, test_repo_path, cleanup_test_repo):
        ...
        assert test_git != main_git, "Test repo and main repo should have different .git directories"
...
class TestGitOperations:
    ...
    def test_can_commit_in_test_repo(self, project_root, test_repo_path, cleanup_test_repo):
    ...
    def test_test_repo_has_no_remote(self, project_root, test_repo_path, cleanup_test_repo):
        ...
        assert result.stdout.strip() == "", "Test repository should have no remotes configured"
...
class TestDocumentation:
    ...
    def test_documentation_content(self, project_root):
        ...
        assert "init_test_artifacts_repo.sh" in content, "Documentation should mention init script"
        assert "cleanup_test_artifacts_repo.sh" in content, "Documentation should mention cleanup script"
