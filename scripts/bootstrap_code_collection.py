#!/usr/bin/env python3
"""
Bootstrap Data Collection Script - Full Pipeline (Planning → Design → Design Review → Code)

Runs the complete ASP pipeline on the 12 bootstrap planning tasks to:
- Generate DesignSpecifications for all bootstrap tasks
- Review each design with the multi-agent Design Review system
- Generate complete production-ready code from approved designs
- Build comprehensive bootstrap telemetry dataset
- Validate end-to-end Planning→Design→Design Review→Code workflow
- Collect cost/time data for PROBE-AI calibration

Usage:
    python scripts/bootstrap_code_collection.py
"""
...
def run_bootstrap_code_collection():
    """Run full pipeline (Planning → Design → Design Review → Code) on all bootstrap tasks."""

    print("="*80)
    print("BOOTSTRAP DATA COLLECTION - Full Pipeline (Planning → Design → Review → Code)")
    print("="*80)
    ...
    output_file = Path("data/bootstrap_code_results.json")
    existing_results = {}
    ...
    # Default coding standards for bootstrap tasks
    ...
    for i, planning_result in enumerate(successful_planning, 1):
        ...
        if task_id in existing_results:
            print(f"\nTASK {i}/{len(successful_planning)}: {task_id} - SKIPPING (already successful)")
            continue
        ...
        try:
            ...
            code_input = CodeInput(
                ...
                design_review_report=review_report,
                coding_standards=default_coding_standards,
            )

            # Execute Code Agent (synchronous)
            generated_code = code_agent.execute(code_input)
            code_elapsed = time.time() - code_start

            # Calculate statistics
            source_files = sum(1 for f in generated_code.files if f.file_type == "source")
            test_files = sum(1 for f in generated_code.files if f.file_type == "test")
            other_files = len(generated_code.files) - source_files - test_files

            print(f"  Code Agent SUCCESS ({code_elapsed:.2f}s)")
            print(f"    Total Files: {generated_code.total_files}")
            print(f"      Source: {source_files}")
            print(f"      Tests: {test_files}")
            print(f"      Other: {other_files}")
            print(f"    Total Lines: {generated_code.total_lines_of_code}")
            print(f"    Dependencies: {len(generated_code.dependencies)}")
            print()

            task_result["code_success"] = True
            task_result["code_execution_time"] = code_elapsed
            task_result["code_total_files"] = generated_code.total_files
            task_result["code_source_files"] = source_files
            task_result["code_test_files"] = test_files
            task_result["code_other_files"] = other_files
            task_result["code_total_lines"] = generated_code.total_lines_of_code
            task_result["code_dependencies"] = len(generated_code.dependencies)

            # Success: All agents completed
            task_result["pipeline_success"] = True
            task_result["total_execution_time"] = design_elapsed + review_elapsed + code_elapsed

        except Exception as e:
            code_elapsed = time.time() - code_start
            print(f"  Code Agent FAILED ({code_elapsed:.2f}s): {e}")
            import traceback
            traceback.print_exc()
            print()

            task_result["code_success"] = False
            task_result["code_execution_time"] = code_elapsed
            task_result["code_error"] = str(e)
            task_result["pipeline_success"] = False
            task_result["total_execution_time"] = design_elapsed + review_elapsed + code_elapsed
...
def main():
    """Main entry point."""
    try:
        results = run_bootstrap_code_collection()
        ...
        if successful:
            ...
            # Code Agent stats
            avg_code_time = sum(r["code_execution_time"] for r in successful) / len(successful)
            avg_total_files = sum(r["code_total_files"] for r in successful) / len(successful)
            avg_source_files = sum(r["code_source_files"] for r in successful) / len(successful)
            avg_test_files = sum(r["code_test_files"] for r in successful) / len(successful)
            avg_total_lines = sum(r["code_total_lines"] for r in successful) / len(successful)

            print(f"\nCode Agent (n={len(successful)}):")
            print(f"  Avg Execution Time: {avg_code_time:.2f}s")
            print(f"  Avg Total Files: {avg_total_files:.1f}")
            print(f"  Avg Source Files: {avg_source_files:.1f}")
            print(f"  Avg Test Files: {avg_test_files:.1f}")
            print(f"  Avg Lines of Code: {avg_total_lines:.1f}")
