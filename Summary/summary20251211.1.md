...
2. ~~**Multi-Language Support**~~ - **OUT OF SCOPE**
   - Additional test parsers (Jest, Go, Cargo) deferred
   - Pytest parser + raw output fallback is sufficient for now
   - If needed later: use JUnit XML (universal format) instead of per-framework parsers
