...
def compare_benchmarks(
    ...
    print(f"{'Benchmark':<60} {'Baseline (ms)':<15} {'Comparison (ms)':<15} {'Change':>10}")
    print("-" * 105)
    ...
    for name in sorted(common_benchmarks):
        ...
        print(f"{display_name:<60} {baseline_mean:>13.4f}  {comparison_mean:>13.4f}  {change_str:>10}")
...
def analyze_by_category(benchmarks: Dict[str, Dict[str, float]]) -> None:
    ...
    for name, stats in benchmarks.items():
        ...
        for category in categories.keys():
            if category in name.lower():
                categories[category].append(stats['mean'])
                categorized = True
                break
        if not categorized:
    ...
    for category, times in categories.items():
        if times:
            ...
            print(f"{category.capitalize():<15} - {len(times):>3} tests, avg: {avg_time:>10.4f} ms")


def main():
    """Main entry point."""
    if len(sys.argv) < 3:
        print("Usage: python compare_results.py <baseline.json> <comparison.json> [<comparison2.json> ...]")
        print("\nExample:")
        print("  python compare_results.py results_311.json results_312.json results_313.json")
        sys.exit(1)
    ...
    for comparison_path in comparison_paths:
        ...
        compare_benchmarks(baseline_stats, comparison_stats, baseline_name, comparison_name)
