...
def manager_routes(app, rt):
    ...
    def get_manager():
        ...
        success_rate = (stats['successful'] / stats['total_tasks'] * 100) if stats['total_tasks'] > 0 else 0
        active_count = len([t for t in tasks if t['status'] in ('in_progress', 'planning')])
        ...
        return Titled("ASP Overwatch - Sarah",
            Div(
                ...
                Div(
                    ...
                    Div(
                        Div(
                            H3(f"{success_rate:.0f}%", cls="pico-color-green" if success_rate >= 80 else "pico-color-yellow"),
                            P("Success Rate"),
                            cls="card", style="text-align: center;"
                        ),
                        Div(
                ...
                Div(
                    ...
                    Div(
                        Div(
                            ...
                            Table(
                                ...
                                Tbody(
                                    Tr(Td("Planning Agent"), Td("Operational", cls="pico-color-green"), Td("Active")),
                                    Tr(Td("Design Agent"), Td("Operational", cls="pico-color-green"), Td("Active")),
                                    Tr(Td("Design Review"), Td("Operational", cls="pico-color-green"), Td("Active")),
                                    Tr(Td("Code Agent"), Td("Operational", cls="pico-color-green"), Td("Active")),
                                    Tr(Td("Code Review"), Td("Operational", cls="pico-color-green"), Td("Active")),
                                    Tr(Td("Test Agent"), Td("Operational", cls="pico-color-green"), Td("Active")),
                                    Tr(Td("Postmortem Agent"), Td("Operational", cls="pico-color-green"), Td("Active")),
                                )
                            ),
                            cls="card",
                    ...
                    Div(
                                ...
                                Tbody(
                                    ...
                                    Tr(
                                        ...
                                        Td(
                                            ...
                                            if review_stats['total_reviews'] > 0 else "N/A",
                        ...
                        Div(
                            ...
                            Ul(
                                *[Li(f"{cat}: {count}") for cat, count in review_stats['by_category'].items()]
                            ) if review_stats['by_category'] else P("No defects categorized", cls="secondary"),
                            cls="card",
                ...
                Div(
                    ...
                    Table(
                        ...
                        Tbody(
                            *[Tr(
                                Td(t['task_id']),
                                Td(t['description'][:35] + "..." if len(t['description']) > 35 else t['description']),
                                Td(str(t['complexity'])),
                                Td(
                                    t['status'],
                                ...
                                Td(f"{t['execution_time']:.2f}s" if t['execution_time'] > 0 else "-")
                            ) for t in tasks[:10]]
                        )
                    ),
                    A("View All Tasks", href="/manager/tasks", role="button", cls="outline"),
                    cls="card",
    ...
    def get_manager_tasks():
        ...
        return Titled("Task Overview - Manager View",
            Div(
                ...
                # Summary stats
                Div(
                    P(f"Total: {len(tasks)} | Completed: {stats['successful']} | "
                      f"Failed: {stats['failed']} | "
                      f"Success Rate: {(stats['successful']/stats['total_tasks']*100):.0f}%"
                      if stats['total_tasks'] > 0 else "No data"),
                ...
                Table(
                    ...
                    Tbody(
                        *[Tr(
                            Td(t['task_id']),
                            Td(t['description'][:50] + "..." if len(t['description']) > 50 else t['description']),
                            Td(str(t['complexity'])),
                            Td(str(t['num_units'])),
                            Td(f"{t['execution_time']:.2f}" if t['execution_time'] > 0 else "-"),
                            Td(t['status'],
                               cls="pico-color-green" if t['status'] == "completed"
                               else "pico-color-azure" if t['status'] == "in_progress"
