...
def product_routes(app, rt):
    ...
    def get_product():
        ...
        return Titled("Project Overview - Jordan",
            Div(
                # Header metrics
                Div(
                        ...
                        Div(
                            ...
                            Div(
                                *[Div(
                                ...
                                P(f"+{len(in_progress)-5} more" if len(in_progress) > 5 else "", cls="secondary")
                            ),
                            style="flex: 1; padding: 0.5rem; background: var(--pico-card-background-color); border-radius: 4px; margin-right: 0.5rem;"
                        ),
                        # Completed column
                        Div(
                            ...
                            Div(
                                *[Div(
                                ...
                                P(f"+{len(completed)-5} more" if len(completed) > 5 else "", cls="secondary")
                            ),
                ...
                Div(
                    ...
                    Div(
                        Div(
                            ...
                            Table(
                                Tbody(
                                    Tr(Td("Total Tasks Processed"), Td(Strong(str(stats['total_tasks'])))),
                                    Tr(Td("Total Units Completed"), Td(Strong(str(stats['total_units'])))),
                                    Tr(Td("Avg Units per Task"), Td(Strong(f"{stats['total_units']/stats['total_tasks']:.1f}" if stats['total_tasks'] > 0 else "N/A"))),
                                )
                            ),
                            cls="card"
                        ),
                        Div(
                            ...
                            Table(
                                Tbody(
                                    Tr(Td("Avg Complexity"), Td(Strong(f"{stats['avg_complexity']:.0f}"))),
                                    Tr(Td("Avg Execution Time"), Td(Strong(f"{stats['avg_execution_time']:.1f}s"))),
                                    Tr(Td("Success Rate"), Td(Strong(f"{(stats['successful']/stats['total_tasks']*100):.0f}%" if stats['total_tasks'] > 0 else "N/A"), cls="pico-color-green")),
                                )
                            ),
                            cls="card"
                        ),
